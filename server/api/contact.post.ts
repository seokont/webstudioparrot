...
export default defineEventHandler(async (event) => {
  ...
  const mailAppPassword = String(config.mailAppPassword || "").replace(
    /\s/g,
    "",
  );
  const recipient = String(
    config.contactRecipient || "maks@seokont.online",
  ).trim();
  ...
  try {
    await transporter.sendMail({
      ...
      subject: sanitizeHeader(
        `Новая заявка SEOKONT — ${lead.type || lead.name}`,
      ),
      text,
      html,
    });
    ...
    throw createError({
      statusCode: 502,
      statusMessage: "Could not deliver contact request",
    });
  }
