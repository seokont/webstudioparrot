...
const localeMessages = computed(
  () => getLocaleMessage(locale.value) as Record<string, any>,
);
const businessProblems = computed(
  () => localeMessages.value?.businessProblems?.items ?? [],
);
const aiWorkflowData = computed(
  () =>
    localeMessages.value?.aiWorkflow ?? {
      sectors: [],
      examples: {},
      steps: [],
    },
...
const advantages = computed(
  () => localeMessages.value?.advantages?.items ?? [],
);
const timeline = computed(() => localeMessages.value?.timeline?.items ?? []);
const technologies = computed(
  () => localeMessages.value?.technologies?.items ?? [],
);
const faqItems = computed(() => localeMessages.value?.faq?.items ?? []);
const projectTypes = computed(
  () => localeMessages.value?.contact?.form?.projectTypes ?? [],
);
const workflowExamples = computed(
  () => Object.values(aiExamples.value) as string[],
);
...
function trapModalFocus(event: KeyboardEvent) {
  ...
  const focusable = Array.from(
    successModalRef.value.querySelectorAll<HTMLElement>(
...
function animateVideoToPointer() {
  ...
  const smoothing = 0.22 + Math.min(0.28, Math.abs(delta) * 0.55);
...
onMounted(() => {
  ...
  if (video) {
    ...
    if (video.readyState >= HTMLMediaElement.HAVE_METADATA) showMiddleFrame();
      ...
      video.addEventListener("loadedmetadata", showMiddleFrame, { once: true });
    ...
    <main>
      <section class="hero" aria-labelledby="hero-title">
        ...
        <div class="container hero-layout">
          <div class="hero-content">
            ...
            <h1 id="hero-title">
              {{ t("hero.line1") }}<br /><span class="accent-word">{{
                t("hero.accent")
              }}</span>
              {{ t("hero.line2") }}
            </h1>
        ...
        <div class="container hero-edge-meta" aria-hidden="true">
          <div class="parrot-label">
            <span class="status-dot" /> {{ t("hero.cursor") }}
          </div>
          <img class="hero-edge-logo" src="/brand/logo.png" alt="" />
        </div>
        <div class="orbit-copy" aria-hidden="true">
          SMART<br />SYSTEMS<br />✦ 2026
        </div>
      </section>
            ...
            >
              <div class="project-visual">
                ...
                <div
                  v-else
                  class="project-art"
                  :data-monogram="item.title.slice(0, 1)"
                />
      ...
      <section id="problems" class="section problems-section">
        <div class="container">
          ...
          <div class="info-grid">
            <article
              v-for="(item, index) in businessProblems"
              :key="index"
      ...
      <section id="workflow" class="section workflow-section">
        <div class="container">
          ...
          <div class="workflow-grid reveal" aria-label="Business sectors">
            <span
              v-for="(sector, index) in aiSectors"
              :key="index"
              class="workflow-tag"
            >
          ...
          <div class="workflow-showcase">
            <div class="workflow-examples">
              <article
                v-for="(text, index) in workflowExamples"
                :key="index"
                class="workflow-example reveal"
              >
                <div class="workflow-example-marker">
                  <span>{{ itemNumber(index) }}</span>
                </div>
                <div>
          ...
          <div class="cta-card reveal">
            ...
            <div class="cta-layout">
              <div class="cta-copy">
                ...
                <a class="cta-link" href="mailto:maks@seokont.online"
                  >maks@seokont.online <ArrowUpRight :size="24"
                /></a>
              </div>
              <form class="contact-form" @submit.prevent="submitContact">
                ...
                <div class="contact-fields contact-fields-two">
                  ...
                  <label>
                    ...
                    <input
                      v-model.trim="contactForm.messenger"
                      name="messenger"
                      autocomplete="off"
                    />
                  </label>
      ...
      <Transition name="contact-modal">
        ...
        >
          ...
          >
            ...
            <span class="contact-modal-icon" aria-hidden="true"
              ><Check :size="30"
            /></span>
            ...
            <a class="contact-modal-email" href="mailto:maks@seokont.online">
              maks@seokont.online <ArrowUpRight :size="18" />
            </a>
            <button
